...
export class InventoryRepository {
    ...
    static async getStock(productId) {
        // stock-in adjustment inc +
        // stock-out adjustment dec -
        ...
        const [increase] = await db
            ...
            .where(
                and(
                    ...
                    or(
                        eq(inventory.type, 'stock_in'),
                        and(
                            eq(inventory.type, 'adjustment'),
                            eq(inventory.adjustment_type, 'increase')
                        )
                    )
                )
            )
        const [decrease] = await db
            ...
            .where(
                and(
                    eq(inventory.product_id, productId),
                    or(
                        eq(inventory.type, 'stock_out'),
                        and(
                            eq(inventory.type, 'adjustment'),
                            eq(inventory.adjustment_type, 'decrease')
                        )
                    )
                )
            )
        const stock =
            Number(increase.total_stock) - Number(decrease.total_stock)
        return stock
    }
